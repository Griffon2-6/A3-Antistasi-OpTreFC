...
_insertionPos = _marker getPos [_displaceDistance, _displaceHeading];
...
[_HEV, _CHUTE_HEIGHT] spawn {
	...
	while {((getPosATL _HEV) select 2) > _CHUTE_HEIGHT} do {
		if (((velocity _HEV) select 2) < -150) then {
			...
			_HEV setVelocity [_xVel, _yVel, -149];
		};
...
// waitUntil {isTouchingGround _HEV && (getPosATL _HEV select 2) < 2.5 || (getPosASL _HEV) select 2 < 1 || vectorMagnitude(velocity _HEV) < 10};
waitUntil {(getPosATL _HEV select 2) < 300 && ((getPosASL _HEV) select 2 < 1 || vectorMagnitude(velocity _HEV) < 50) || isTouchingGround _HEV};

_pos = getPosVisual _HEV;

if (!(surfaceIsWater _pos)) then {
	// _anchor = createVehicle ["Sign_Sphere10cm_F", [_pos select 0, _pos select 1, 0], [], 0, "NONE"];
	_anchor = createVehicle ["Sign_Sphere10cm_F", [_pos select 0, _pos select 1, worldSize], [], 0, "NONE"];
	_anchor hideObjectGlobal true;

	_anchor setDir (getDir _HEV);
	// _anchor setPosATL [_pos select 0, _pos select 1, 0];
	_anchor setVehiclePosition [[_pos select 0, _pos select 1, worldSize], [], 0, "NONE"];
